...
export interface DashboardSummary {
  ...
  unbillable_count: number;
  over_allocated_count: number;
  // Revenue leakage
  unbillable_leakage_monthly: number;
  overalloc_leakage_monthly: number;
...
export function useDataHealth() {
  return useQuery<DataHealth>({
    ...
    queryFn: () => api.get("/api/dashboard/data-health").then((r) => r.data),
  });
}
...
export interface ExtensionConflict {
  ...
  current_alloc_end: string;
  current_coe: string | null;
...
export interface ExtensionConflictsResponse {
...
export function useExtensionConflicts() {
  return useQuery<ExtensionConflictsResponse>({
    queryKey: ["extension-conflicts"],
    queryFn: () => api.get("/api/rmg/extension-conflicts").then((r) => r.data),
  });
}

export interface ExtensionConfirmation {
  id: number;
  project_id: string;
  client_id: string;
  project_end_date: string | null;
  ...
  response: "no_extension" | "extend" | "partial" | null;
  responded_at: string | null;
  staying_employee_ids: string[] | null;
  new_end_date: string | null;
}

export function useExtensionConfirmations() {
  return useQuery<ExtensionConfirmation[]>({
    queryKey: ["extension-confirmations"],
    queryFn: () => api.get("/api/rmg/extension-confirmations").then((r) => r.data),
  });
}
