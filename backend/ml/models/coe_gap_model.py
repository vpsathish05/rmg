...
def _get_coe_headcount(db: Session) -> dict[str, int]:
    ...
    Falls back to canonical_role-based inference for employees with no COE tag.
    """
    ...
    # Infer COE from canonical_role for employees with no skill-based COE tag
    # This catches Consultant-type roles that are untagged in employee_skills
    role_inferred = db.execute(text("""
        SELECT e.canonical_role, COUNT(*) AS hc
        FROM employees e
    ...
    for r in role_inferred:
        inferred_coe = _infer_coe_from_role(r.canonical_role)
        if inferred_coe:
            result[inferred_coe] = result.get(inferred_coe, 0) + int(r.hc)
...
def _get_coe_billable(db: Session) -> dict[str, int]:
    ...
    # Infer COE from canonical_role for billable employees with no skill-based COE tag
    role_inferred = db.execute(text("""
        SELECT e.canonical_role, COUNT(DISTINCT e.employee_id) AS hc
        FROM employees e
        WHERE e.is_active_version = true AND e.account_status = true
          AND e.date_of_resignation IS NULL
...
def _infer_coe_from_role(canonical_role: str) -> str | None:
    """
    Infer COE from canonical_role for employees with no skill-based COE tag.
    Uses the same logic as pipeline_calc._infer_coe() to ensure supply/demand alignment.
    
    Only maps roles that generate pipeline DEMAND for a given COE - this ensures
    supply and demand are measured with the same ruler.
    """
    if not canonical_role:
        return None
    role_lower = canonical_role.lower()
    # Only infer for delivery roles - skip trainees (they inflate supply unrealistically)
    if "trainee" in role_lower:
        return None
