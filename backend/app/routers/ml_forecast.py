...
def _gbp(usd: float | None) -> float:
    """Convert a USD-denominated model output to GBP using the rate card's FX rate."""
    return round((usd or 0) / FX_GBP_TO_USD, 2)
...
def get_revenue_forecast(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "last_actual_value": _gbp(result.last_actual_value),
        "annual_total_p50": _gbp(result.annual_total_p50),
        "growth_rate_yoy": result.growth_rate_yoy,
        "train_mape": result.train_mape,
        "trend_monthly": _gbp(result.trend_monthly),
        "forecasts": [
            {
                "month": f.month,
                "p10": _gbp(f.p10),
...
def get_cluster_forecast(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "total_by_cluster": {cl: _gbp(v) for cl, v in result.total_by_cluster.items()},
        "clusters": {
            cl: [
                {
                    "month": cm.month,
                    "weight": cm.weight,
                    "revenue_p10": _gbp(cm.revenue_p10),
                    "revenue_p50": _gbp(cm.revenue_p50),
                    "revenue_p90": _gbp(cm.revenue_p90),
                }
                for cm in months
            ]
...
def get_forecast_summary(db: Session = Depends(get_db)):
    ...
    return {
        "revenue": {
            "annual_total_p50": _gbp(rev.annual_total_p50),
            "growth_rate_yoy": rev.growth_rate_yoy,
            "next_month_p50": _gbp(rev.forecasts[0].p50 if rev.forecasts else 0),
            "last_actual": _gbp(rev.last_actual_value),
            "trend_monthly": _gbp(rev.trend_monthly),
        },
        "projects": {
        ...
        "pipeline": {
            "total_weighted_revenue": _gbp(pipeline.total_weighted_revenue),
